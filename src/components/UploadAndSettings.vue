...
const localImagePreview = ref(props.initialImageBase64);
const localDesignFile = ref(null);
const localForm = reactive({ ...props.initialFormSettings });
...
watch(() => props.initialImageBase64, (newVal) => {
  ...
  if (newVal) {
    localFile.value = null; // Clear file object if an initial image is loaded from session
...
function handleGenerateClick() {
  if (!localFile.value && !localImagePreview.value) {
    ElMessage.warning('请先上传一张图片或设计文件。');
    return;
  }
  emit('generate-prompt-requested', {
    imageBase64: localImagePreview.value,
    appType: 'web', // This seems fixed for now, can be added to form if needed
    temperature: localForm.temperature / 100,
    framework: localForm.framework,
    componentLibrary: localForm.componentLibrary,
    newRawFileObject: localFile.value ? localFile.value.raw : null,
...
.controls-card {
  ...
  /* Remove height: 100% to allow content to expand naturally */
}
.controls-card :deep(.el-card__body) {
  padding: 0 !important; display: flex; flex-direction: column; flex-grow: 1;
}
.card-content {
  display: flex;
...
.uploader :deep(.el-icon--upload) {
  font-size: 56px; color: var(--color-text-tertiary); margin-bottom: 20px; transition: all var(--transition-base);
}
.uploader :deep(.el-upload-dragger:hover .el-icon--upload) {
  color: var(--color-primary); transform: scale(1.05);
}
.uploader :deep(.el-upload__text) { color: var(--color-text-secondary); font-size: 16px; }
.uploader :deep(.el-upload__text em) { color: var(--color-primary); font-style: normal; }
.uploader :deep(.el-upload__tip) { color: var(--color-text-tertiary); font-size: 13px; margin-top: 10px; }

.image-preview, .design-file-preview {
  position: relative; width: 100%; height: 250px; overflow: hidden;
  border-radius: var(--border-radius-lg); display: flex; align-items: center; justify-content: center;
  background: var(--color-bg-medium); border: 1px solid var(--color-bg-light);
}
...
.image-preview:hover .image-actions, .design-file-preview:hover .file-actions { opacity: 1; }

.design-file-preview { flex-direction: column; gap: 1rem; text-align: center; }
.file-icon .el-icon { font-size: 56px; color: var(--color-text-tertiary); }
.file-info .file-name { font-size: 16px; margin: 0; color: var(--color-text-primary); }
...
.temperature-value { font-weight: 500; color: var(--color-text-secondary); }
.temperature-slider { display: flex; align-items: center; gap: 12px; }
.slider-label { font-size: 13px; color: var(--color-text-tertiary); }
.temperature-slider .el-slider { flex-grow: 1; }

.setting-desc { font-size: 12px; color: var(--color-text-tertiary); margin-top: 8px; }
...
.api-key-warning { margin-bottom: 16px; }
.generate-button {
  width: 100%; height: 48px; font-size: 16px;
}
.generate-button .el-icon { font-size: 18px; margin-right: 8px; }
.full-width { width: 100%; }
.full-width .el-icon { margin-right: 6px; }
</style>
